...
static void update_AXI_Register(handler_PWMManager *handler, IP_registers reg)
{
	...
	switch(reg)
	{
		...
		case slv_reg4:
			regInfo = handler->slv_reg4.AXI_X32_Register;
			break;

		case slv_reg5:
			regInfo = handler->slv_reg5.AXI_X32_Register;
			break;

		case slv_reg6:
			regInfo = handler->slv_reg6.AXI_X32_Register;
			break;

		case slv_reg7:
			regInfo = handler->slv_reg7.AXI_X32_Register;
			break;
...
{
	handler->slv_reg1.AXI_Sub_Registers.LSR = PWM_Value;
	update_AXI_Register(handler, slv_reg1);
}

void set_Max_PWM(handler_PWMManager *handler, u16 PWM_Value)
{
	handler->slv_reg1.AXI_Sub_Registers.MSR = PWM_Value;
	update_AXI_Register(handler, slv_reg1);
}
...
void set_Inversion(handler_PWMManager *handler, channel ch, inversion inv)
{
	...
	{
		case ch_Right:
			handler->slv_reg0.AXI_Bits.b1 = (inv == channel_Inverted) ? TRUE : FALSE;
			break;

		case ch_Left:
			handler->slv_reg0.AXI_Bits.b0 = (inv == channel_Inverted) ? TRUE : FALSE;
			break;
...
void set_Direct_Value(handler_PWMManager *handler, channel ch, u16 PWM_Value)
{
	...
	{
		case ch_Right:
			handler->slv_reg2.AXI_Sub_Registers.MSR = PWM_Value;
			break;

		case ch_Left:
			handler->slv_reg2.AXI_Sub_Registers.LSR = PWM_Value;
			break;
...
void set_Output_Type(handler_PWMManager *handler, channel ch, outputType outT)
{
	...
	{
		case ch_Right:
			handler->slv_reg0.AXI_Bits.b3 = (outT == pwm_out_direct) ? TRUE : FALSE;
			break;

		case ch_Left:
			handler->slv_reg0.AXI_Bits.b2 = (outT == pwm_out_direct) ? TRUE : FALSE;
			break;
...
{
	handler->slv_reg7.AXI_Sub_Registers.LSR = actuation;
	update_AXI_Register(handler, slv_reg7);
}

u16 get_RC_Input_Value(handler_PWMManager *handler)
{
	handler->slv_reg3.AXI_X32_Register = read_AXI_Register(handler->baseAddr, slv_reg3);
	return handler->slv_reg3.AXI_Sub_Registers.LSR;
}

u16 get_Inverter_Value(handler_PWMManager *handler)
{
	handler->slv_reg3.AXI_X32_Register = read_AXI_Register(handler->baseAddr, slv_reg3);
	return handler->slv_reg3.AXI_Sub_Registers.MSR;
}

u16 get_Output_Value(handler_PWMManager *handler, channel ch)
{
	u16 temp_value = 0;
	handler->slv_reg4.AXI_X32_Register = read_AXI_Register(handler->baseAddr, slv_reg4);
	switch(ch)
	{
		case ch_Right:
			temp_value = handler->slv_reg4.AXI_Sub_Registers.MSR;
			break;

		case ch_Left:
			temp_value = handler->slv_reg4.AXI_Sub_Registers.LSR;
			break;
...
{
	return handler->slv_reg1.AXI_Sub_Registers.MSR;
}
...
inversion get_Inversion(handler_PWMManager *handler, channel ch)
{
	...
	{
		case ch_Right:
			temp_value = (handler->slv_reg0.AXI_Bits.b1 == TRUE) ? channel_Inverted : channel_NotInverted;
			break;

		case ch_Left:
			temp_value = (handler->slv_reg0.AXI_Bits.b0 == TRUE) ? channel_Inverted : channel_NotInverted;
			break;
...
outputType get_Output_Type(handler_PWMManager *handler, channel ch)
{
	...
	{
		case ch_Right:
			temp_value = (handler->slv_reg0.AXI_Bits.b3 == TRUE) ? pwm_out_direct : pwm_out_processed;
			break;

		case ch_Left:
			temp_value = (handler->slv_reg0.AXI_Bits.b2 == TRUE) ? pwm_out_direct : pwm_out_processed;
			break;
